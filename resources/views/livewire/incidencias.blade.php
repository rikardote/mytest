<div class="mt-8 min-h-screen">
    <div class="flex justify-between">
        <button type="submit" wire:click="$emitTo('incidencias-child', 'showCreateForm');" class="text-blue-500">
        ...
        <table class="w-full whitespace-no-wrap mt-4 shadow-2xl" wire:loading.class.delay="opacity-50">
            <thead>
                ...
                <td class="px-3 py-2" >
                    <div class="flex items-center">
                        <button wire:click="sortBy('code')">Codigo</button>
                        <x:tall-crud-generator::sort-icon sortField="code" :sort-by="$sortBy" :sort-asc="$sortAsc" />
                    </div>
                </td>
                <td class="px-3 py-2" >
                    <div class="flex items-center">
                        <button wire:click="sortBy('description')">Descripcion</button>
                        <x:tall-crud-generator::sort-icon sortField="description" :sort-by="$sortBy" :sort-asc="$sortAsc" />
                    </div>
                </td>
            ...
            @foreach($results as $result)
                <tr class="hover:bg-blue-300 {{ ($loop->even ) ? "bg-blue-100" : ""}}">
                    ...
                    <td class="px-3 py-2" >
                        <button type="submit" wire:click="$emitTo('incidencias-child', 'showEditForm', {{ $result->id}});" class="text-green-500">
                            <x:tall-crud-generator::icon-edit />
                        </button>
                        <button type="submit" wire:click="$emitTo('incidencias-child', 'showDeleteForm', {{ $result->id}});" class="text-red-500">
